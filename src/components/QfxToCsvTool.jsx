...
export default function QfxToCsvTool() {
  ...
  const onDrop = async (e) => {
    ...
    if (ext !== 'qfx' && ext !== 'ofx') {
      setError("Please upload a valid .qfx or .ofx Web Connect statement.");
      return;
    }
    ...
    try {
      ...
      if (tableData && tableData.length > 1) {
        ...
        setError("No transactions found in this QFX/OFX file. Make sure it's not empty or corrupted.");
      }
    } catch (err) {
      console.error(err);
      setError("Failed to read file. Please ensure it's a valid text-based QFX/OFX statement.");
    } finally {
    ...
    <div className="converter-card" style={{ maxWidth: '850px', margin: '0 auto' }}>
      ...
      {!csvData && !isProcessing && (
        ...
        >
          ...
          <h3>Drag & drop your Quicken .qfx or .ofx file</h3>
          <p>Extract QFX statement data locally in seconds</p>
          <div className="file-types">
            <span className="file-type-tag">.qfx</span>
      ...
      {isProcessing && (
        <div className="loader">
          ...
          <h3>Reading Quicken Web Connect statement...</h3>
      ...
      <div className="tool-footer" style={{ borderTop: '1px solid var(--border)', paddingTop: '1.5rem', display: 'flex', justifyContent: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.85rem', color: 'var(--slate-600)' }}>
          ...
          <span>Local processing: Your QFX data never leaves your computer.</span>
        </div>
      </div>
    </div>
  );
